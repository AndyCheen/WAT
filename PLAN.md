...
`currentStreak, longestStreak, lastCountedDayKey, frozenDayKeys, lastFreezeUsedDayKey, updatedAt`
· `freezeTokens` лишається в схемі, але з WAT-34 не використовується: готова заморозка — це сам `RewardItem`.
...
**`RewardDefinition`** (каталог) `key, kind (theme|xpBoost|streakFreeze), titleKey, descKey, icon` — у каталозі лише 🧊 `streak.freeze` і ⚡ `xp.double` (SPEC-PRIZES §3)

**`RewardItem`** — видана нагорода / приз
`id, defKey, source (level|quest|achievement|random|seed), acquiredAt, activatedAt?, expiresAt?, state (ready|active|used|expired), seenAt?, usedOnDayKey?`
· прострочення буста похідне від `clock.now`; використані записи не видаляються (на них тримаються `XPEntry` і повернення заморозки).
...
→ «ЗАВДАННЯ» (сьогодні / цього тижня) → «ПРИЗИ» з кнопкою «Активувати» → «ДОСЯГНЕННЯ» —
вітрина до 4×2 (лише відкриті й ті, що в процесі), тап по бейджу → картка деталей, «Всі» → **2e**.
Дані: `LevelState`, `XPEntry`, `QuestInstance`, `RewardItem`, `AchievementProgress`, `RewardDefinition`.

### 7.3 Макет 2e — Досягнення

Лічильник «ВІДКРИТО / N / M» → один ряд фільтрів (чипи стану + меню категорій від кнопки)
→ сітка плиток 3×N (закриті — з прогрес-баром і `val/target`) → картка деталей `WTAchievementDetail`.
Реалізовано за ТЗ у WAT-23.

Детальне ТЗ модуля (два фільтри, сортування, секретні, картка деталей, порожні стани,
доступність, перелік змін у коді) — **`SPEC-ACHIEVEMENTS.md`**, макет — `Design/Achievements.html`.

### 7.3a Модуль «Призи» (WAT-26 → реалізовано в WAT-34)

До WAT-34 блок «ПРИЗИ» працював лише формально: 3 з 5 призів після «Активувати» нічого не роблять,
а заморозка серії ніколи не витрачається (`StreakEngine.useFreeze` без викликів). ТЗ переробляє
модуль: приз — предмет із дією; у каталозі 🧊 заморозка серії й ⚡ подвійний XP до кінця доби.
Показуються лише отримані й не використані призи (діючий — статусом у заголовку блоку на 3f і hero-карткою на екрані «Призи», однакові — стосом «×N»),
історії немає; новий екран «Призи» (Діє зараз / Готові); дія — лише в картці призу; кнопка
заморозки — «вчора» чи «сьогодні» за правилом §6.2. Головний екран не змінюється.
Детально — **`SPEC-PRIZES.md`**, макет — `Design/Prizes.html`.
...
| **6a** | Модуль «Призи» | ✅ | заморозка серії й подвійний XP, блок 3f, екран «Призи», картка з дією (WAT-34) |
| **7** | Екран 2e | ✅ | лічильник, фільтри стану й категорії, сітка 3×N, картка деталей, тост розблокування (WAT-23) |
...
**Тести:** 178 unit (8 пакетів) + 17 e2e у симуляторі — усі зелені.
...
10. **Заморозка серії існувала лише на папері (WAT-34).** Приз «активувався» у жетон
    `freezeTokens`, який ніхто не читав, а `useFreeze` не мав жодного виклику. Два шари для
    однієї сутності прибрано: предмет в інвентарі і є жетон, `useFreeze(prizeId:)` сам обирає
    день за правилом `StreakEngine.freezeTarget` (SPEC-PRIZES §6.2, дослівно в doc-коментарі).
    Заодно заморожені дні перестали **додавати** до серії — вони лише тримають ланцюг,
    а заморожений день, який потім закрили, повертає приз в інвентар.
11. **Повернута заморозка не могла б спрацювати вдруге.** Шина метрик ідемпотентна за
    `name + sourceRef`, а `prize.activated` пишеться з `sourceRef = id предмета`. Тож при
    поверненні призу метрика активації відкочується — інакше повторне використання того
    самого предмета мовчки не лягло б у статистику.
...
5. Звуки (`soundEnabled` у профілі теж поки без керування).
...
8. **Призи — відкриті питання** `SPEC-PRIZES.md` §13: призи за завдання й випадкові, тема-приз
   (палітра «Океан»), «Знавець гідратації» як досягнення, ліміт заморозок (зараз за рівень 30
   набереться 10), заморозка «сьогодні» без серії.
