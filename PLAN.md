...
| **9** | Зшивання й полірування | 🟡 | навігація, теми й e2e готові; лишились шрифти, локалізація в каталог, гаптика |

**Тести:** 105 unit (8 пакетів) + 9 e2e у симуляторі — усі зелені.
...
4. **Ідентифікатор доступності на контейнері перекриває дочірні.** Через це шторки були
   ...
   тільки на конкретних елементах.
...
1. Локалізація: винести рядки в `.xcstrings` (зараз — у коді, українською).
2. Гаптика й звуки, порожні стани для 2e/3f.
3. Темна тема для 2e / 3f / 4a — прев'ю-прохід і звірка.
4. Далі за планом: онбординг 5a–5f → сповіщення → віджети.
5. Коли дійде черга до формули норми — увімкнути `GoalCalculatorSheet` (один рядок на екрані 4a).
